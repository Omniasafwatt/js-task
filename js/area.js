async function getmeals(){
    let mealsresponse = await fetch('https://www.themealdb.com/api/json/v1/1/list.php?a=list')
...
function displaydata(meals){
    ...
    for(let i=0 ; i< meals.length ;i++){
        cartona +=`
        <div onclick=getmealsdata('${meals[i].strArea}') class="col-md-3 text-center">
        <i class="icon fa-solid fa-house-laptop fa-4x"></i>
        <h2 class="icon">${meals[i].strArea}</h2>
    </div>
...
async function getmealsdata(typemeal){
    let mealsresponse = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${typemeal}`)
    mealdata = await mealsresponse.json()

...
async function getmeals(mealname){
    let mealsresponse = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${mealname}`)
...
searchbyname.addEventListener('input',function(){
        getmeals(searchbyname.value);

    })


async function getmealsdata(meal){
    let mealsresponse = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?f=${meal}`)
    mealdata = await mealsresponse.json()
    meals = mealdata.meals
    // console.log(meals)
    displaydata(meals)
...
let searchbyletter = document.getElementById('search-by-letter')
searchbyletter.addEventListener('input',function(){
        getmealsdata(searchbyletter.value);

    })
// $('#search-by-letter').on('input', function(){
//     getmeals($('#search-by-letter').value)
// })
function displaydata(meals){
    cartona=``
    for(let i=0 ; i< meals.length ;i++){
        cartona +=`
        <div onclick=getmealsdetails('${meals[i].idMeal}') class="sora col-md-3">
        <div class="meal rounded-2">
        <img src="${meals[i].strMealThumb}" class=" image-meal w-100" alt="">
        <div class="slideUp">
...
// // let meal = finaldata.strMeal
// // console.log(meal)
// searchbyname.addEventListener('input',function(){
//     start(searchbyname.value)
// })
